 ...
 *  Zipato Shutter
 *
 *  Copyright 2019 SwitchingFromZipato
 *
...
metadata {
    definition (name: "Zipato Shutter", namespace: "h8pewou", author: "SwitchingFromZipato", ocfDeviceType: "oic.d.blind") {
...
private handleLevelReport(physicalgraph.zwave.Command cmd) {
    ...
    def zipatolevel = (level - 100) * -1 as Integer
    level = zipatolevel
    if (level >= 99) {
        level = 100
        shadeValue = "open"
...
def setLevel(value, duration = null) {
    ...
    Integer level = (value - 100) * -1 as Integer
    if (level < 0) level = 0
